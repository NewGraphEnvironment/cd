...
additional variables (vapour pressure deficit and relative humidity from
temperature + dewpoint via the Tetens equation, soil moisture as a
4-depth mean), and writes Cloud-Optimized GeoTIFFs alongside a static
...
`cd_trend`, `cd_compare`, `cd_summary`) read the COGs directly via
GDAL’s `/vsicurl/` — no credentials, no tile server — crop to a
user-supplied area of interest, compute baselines and anomalies for
arbitrary reference periods, and run Mann-Kendall and Theil-Sen trend
statistics. All baseline and comparison logic stays on the consumer
side, so reference periods are not baked into the served data.
